...
public class ChimpsSquaresScript : MonoBehaviour
{
    ...
    private TextMeshProUGUI m_displayNumber;
    ...
    private void Awake()
    {
        m_displayNumber = GetComponentInChildren<TextMeshProUGUI>();
    ...
    void Update()
    {
        if (self.GetComponent<Canvas>().enabled)
        {
            if (!m_numbersVisible)
            {
                self.GetComponent<Canvas>().enabled = false;
            }
        }
    }
